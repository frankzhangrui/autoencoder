...
void weightRandomInit(SMR &smr, int nclasses, int nfeatures, double epsilon){
...
void softmaxRegressionCost(Mat &x, Mat &y, SMR &smr, double lambda){
...
void fineTuneNetworkCost(Mat &x, Mat &y, vector<SA> &hLayers, SMR &smr, double lambda){
...
void gradientChecking(SMR &smr, Mat &x, Mat &y, double lambda){
...
void gradientChecking(vector<SA> &hLayers, SMR &smr, Mat &x, Mat &y, double lambda){
...
void trainSoftmaxRegression(SMR& smr, Mat &x, Mat &y, double lambda, double lrate, int MaxIter){
    int nfeatures = x.rows;
    int nsamples = x.cols;
...
void trainFineTuneNetwork(Mat &x, Mat &y, vector<SA> &HiddenLayers, SMR &smr, double lambda, double lrate, int MaxIter){
...
Mat resultProdict(Mat &x, vector<SA> &hLayers, SMR &smr){
...
main(int argc, char** argv)
{
    ...
    for(int i=0; i<SparseAutoencoderLayers; i++){
        ...
        SA tmpsa;
        trainSparseAutoencoder(tmpsa, tempX, 600, 3e-3, 0.1, 3, 2e-2, 400);
    ...
    // Now train Softmax.
    SMR smr;
    trainSoftmaxRegression(smr, Activations[Activations.size() - 1], trainY, 3e-3, 2e-2, 400);
    // Finetune using Back Propogation
    trainFineTuneNetwork(trainX, trainY, HiddenLayers, smr, 1e-4, 2e-2, 400);
